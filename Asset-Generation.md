...
Variant-CIT's virtual assets are known to be incompatible with ModerFix's [Dynamic Resources](https://github.com/embeddedt/ModernFix/wiki/Dynamic-Resources-FAQ) feature.
A simple workaround is to turn those assets into a regular resource pack.

The command `/variants-cit assetgen createPack` will create and load a regular resource pack called **"VCIT Baked AssetGen"**, which includes all assets generated by all modules from all currently loaded packs.
...
Resource-pack makers can copy those assets into their own packs, to ensure they work out of the box in any scenario. Be careful not to overwrite your own assets in the process; in case of conflicts, prefer your own assets over the baked ones.
...
You can use the command `/variants-cit assetGen peek` to quickly check the content of any generated asset.
...
A generator can only generate assets that can collected by its module. I.e: assets matching the `modelPrefix`, the `fallback` model, and the `special` models. There is no need to write a regex specifically to exclude other assets from being collected. (See also: [Radical](#field-radicalpath))
...
"Radical" here is to be understood as "linguistic root". It can be thought of as having the same role as a "variant ID", except it can also be used to designate fallback and special models.
